...
export default {
    ...
    new Promise((resolve, reject) => {
      try {
        ...
        Model.findAndCountAll(jobs, {
          ...
          .catch((err) => {
            reject(ErrorHelpers.errorReject(err, "getListError", "jobservice"));
          });
      } catch (err) {
        reject(ErrorHelpers.errorReject(err, "getListError", "jobservice"));
      }
    ...
    new Promise((resolve, reject) => {
      try {
        ...
        Model.findOne(jobs, {
          ...
          .catch((err) => {
            reject(ErrorHelpers.errorReject(err, "getInfoError", "jobservice"));
          });
      } catch (err) {
        reject(ErrorHelpers.errorReject(err, "getInfoError", "jobservice"));
      }
  ...
  create: async (param) => {
    ...
    try {
      ...
      finnalyResult = await Model.create(jobs, param.entity).catch((error) => {
        throw new ApiErrors.BaseError({
          statusCode: 202,
          type: "crudError",
          error,
      ...
      ErrorHelpers.errorThrow(error, "crudError", "jobservice");
    }
  ...
  update: async (param) => {
    ...
    try {
      ...
      const foundTopic = await Model.findOne(jobs, {
        where: {
          id: param.id,
        },
      }).catch((error) => {
      ...
      if (foundTopic) {
        ...
        await Model.update(jobs, entity, {
        ...
        finnalyResult = await Model.findOne(jobs, {
          where: { Id: param.id },
        }).catch((error) => {
          throw new ApiErrors.BaseError({
            statusCode: 202,
  ...
  delete: async (param) => {
    try {
      ...
      if (!foundTopic) {
        ...
        await Model.destroy(jobs, { where: { id: parseInt(param.id) } });

        const wardAfterDelete = await Model.findOne(jobs, {
          where: { Id: param.id },
        }).catch((err) => {
          ErrorHelpers.errorThrow(err, "crudError", "jobservice");
        });
