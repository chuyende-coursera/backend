...
export default {
  get_list: (param) =>
    new Promise((resolve, reject) => {
      try {
        ...
        Model.findAndCountAll(languages, {
          where: whereFilter,
          order: [sort],
          offset: range[0],
          limit: perPage,
          ...
          .catch((err) => {
            reject(
              ErrorHelpers.errorReject(err, "getListError", "languageservice")
            );
          });
      } catch (err) {
        reject(
    ...
    new Promise((resolve, reject) => {
      try {
        ...
        Model.findOne(languages, {
          ...
          .catch((err) => {
            reject(
              ErrorHelpers.errorReject(err, "getInfoError", "languageservice")
            );
          });
      } catch (err) {
        reject(
  ...
  create: async (param) => {
    ...
    try {
      ...
      finnalyResult = await Model.create(languages, param.entity).catch(
        (error) => {
          throw new ApiErrors.BaseError({
            statusCode: 202,
            type: "crudError",
      ...
      ErrorHelpers.errorThrow(error, "crudError", "languageservice");
    }
  ...
  update: async (param) => {
    ...
    try {
      ...
      const foundTopic = await Model.findOne(languages, {
        where: {
          id: param.id,
        },
      }).catch((error) => {
      ...
      if (foundTopic) {
        ...
        await Model.update(languages, entity, {
        ...
        finnalyResult = await Model.findOne(languages, {
          where: { Id: param.id },
        }).catch((error) => {
          throw new ApiErrors.BaseError({
            statusCode: 202,
  ...
  delete: async (param) => {
    try {
      ...
      if (!foundTopic) {
        ...
        await Model.destroy(languages, { where: { id: parseInt(param.id) } });

        const wardAfterDelete = await Model.findOne(languages, {
          where: { Id: param.id },
        }).catch((err) => {
          ErrorHelpers.errorThrow(err, "crudError", "languageservice");
        });
