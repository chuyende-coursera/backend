...
const { categories, topics, courses } = models;
...
export default {
    ...
    new Promise((resolve, reject) => {
      try {
        ...
        Model.findAndCountAll(categories, {
          ...
          .catch((err) => {
            reject(
              ErrorHelpers.errorReject(err, "getListError", "categoryService")
            );
          });
      } catch (err) {
        reject(
    ...
    new Promise((resolve, reject) => {
      try {
        ...
        Model.findOne(categories, {
          ...
          .catch((err) => {
            reject(
              ErrorHelpers.errorReject(err, "getInfoError", "categoryService")
            );
          });
      } catch (err) {
        reject(
  ...
  create: async (param) => {
    ...
    try {
      ...
      finnalyResult = await Model.create(categories, param.entity).catch(
        (error) => {
          throw new ApiErrors.BaseError({
            statusCode: 202,
            type: "crudError",
      ...
      ErrorHelpers.errorThrow(error, "crudError", "categoryService");
    }
  ...
  update: async (param) => {
    ...
    try {
      ...
      if (foundCategory) {
        ...
        //           districtsId: entity.districtsId || foundCategory.districtsId,
        //         },
        //       },
        ...
        await Model.update(categories, entity, {
    ...
    try {
      ...
      const foundCategory = await Model.findOne(categories, {
        ...
        throw new ApiErrors.BaseError({
          statusCode: 202,
          type: "getInfoError",
          message: "Lấy thông tin của danh mục thất bại!",
          error,
        });
      });
      ...
      if (!foundCategory) {
        ...
        await Model.destroy(categories, { where: { id: parseInt(param.id) } });

        const wardAfterDelete = await Model.findOne(categories, {
          where: { Id: param.id },
        }).catch((err) => {
          ErrorHelpers.errorThrow(err, "crudError", "categoryService");
        });
